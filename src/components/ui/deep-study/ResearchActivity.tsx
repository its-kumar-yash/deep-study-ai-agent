...
export default function ResearchActivity() {
  const { activities, sources } = useDeepStudyStore();
    ...
    <div className="w-[90vw] sm:w-[400px] fixed top-4 right-4 z-20">
      <Collapsible className="w-full" open={isOpen} onOpenChange={setIsOpen}>
        ...
        <CollapsibleContent className="h-[50vh]">
          <Tabs defaultValue="activities" className="w-full h-full shadow-md rounded-[14px]">
            <TabsList className="w-full flex gap-0.5 px-2 py-6">
              <TabsTrigger
                value="activities"
                className="flex-1 p-4 shadow-none"
              >
                Activities
              </TabsTrigger>
              {sources.length > 0 && (
                <TabsTrigger value="sources" className="p-4">Sources</TabsTrigger>
              )}
            </TabsList>
            <TabsContent
              value="activities"
